 ...
 * work to an internal TelldusServer instance.
 *
 ...
 * The telldusServer socket is only used when a explicit command is executed.
 *
 ...
 *
 * Notifications are delivered to the telldusServer app via Listeners. The telldusServer provides
 ...
 */
public class Telldus implements TelldusInterface {
    protected TelldusServer telldusServer;

    public Telldus(String host, int clientPort, int eventPort) throws IOException {
        telldusServer = new TelldusServer(host, clientPort, eventPort);
    }
    ...
    public void registerListener(TelldusEvent.Listener listener) {
        telldusServer.registerEventListener(listener);
    }
    ...
    public void unregisterListener(TelldusEvent.Listener listener) {
        telldusServer.unregisterEventListener(listener);
    }
    ...
    public int tdTurnOn(int deviceId) {
        ...
        return telldusServer.getIntegerFromService(m);
    }
     ...
     * @param methodsSupported The methods supported by the telldusServer. See tdMethods() for more information.
     *
     * @return the last sent command as Protocol.DeviceMethod.
     */
    public Protocol.DeviceMethod tdLastSentCommand(int deviceId, int methodsSupported) {
        Message m = new Message("tdLastSentCommand");
        m.addArgument(deviceId);
        m.addArgument(methodsSupported);
        return Protocol.DeviceMethod.fromCode(telldusServer.getIntegerFromService(m));
    }
    ...
    public int tdGetNumberOfDevices() {
        return telldusServer.getIntegerFromService(new Message("tdGetNumberOfDevices"));
    }
     ...
     *
     * int intNumberOfDevices = telldusServer.tdGetNumberOfDevices();
     * for (int i = 0; i < intNumberOfDevices; i++) {
     *   int id = telldusServer.tdGetDeviceId( i );
    ...
    public int tdGetDeviceId(int intDeviceIndex) {
        ...
        return telldusServer.getIntegerFromService(msg);
    }
    ...
    public Protocol.DeviceType tdGetDeviceType(int intDeviceId) {
        ...
        return Protocol.DeviceType.fromCode(telldusServer.getIntegerFromService(msg));
    }
    ...
    public String tdGetName(int intDeviceId) {
        ...
        msg.addArgument(intDeviceId);
        return telldusServer.getStringFromService(msg);
    }
    ...
    public boolean tdSetName(int intDeviceId, String strNewName) {
        ...
        return telldusServer.getBoolFromService(msg);
    }
     ...
     * //methods is now DeviceMethod.TURNOFF.code() because the telldusServer application doesn't support BELL
     *
     * // XXX JTelldus: Should clean this up with array of enum or something..
     *
     * @param id The device id to query
     * @param methodsSupported The methods the telldusServer application supports
     *
     * @return The method-flags Protocol.DeviceMethod codes OR'ed into an integer.
     */
    public int tdMethods(int id, Protocol.DeviceMethod... methodsSupported) {
        int m = 0;
        for(Protocol.DeviceMethod dm: methodsSupported)
            m+= dm.code();
    ...
    public String tdGetErrorString(int intErrorNo) {
        Protocol.ErrorCode c = Protocol.ErrorCode.fromCode(intErrorNo);
        return c.str();
    }
     ...
     */
    public Protocol.ErrorCode tdSendRawCommand(String command, int reserved) {
        Message msg = new Message("tdSendRawCommand");
        msg.addArgument(command);
        msg.addArgument(reserved);
        return Protocol.ErrorCode.fromCode(telldusServer.getIntegerFromService(msg));
    }
    ...
    public void tdConnectTellStickController(int vid, int pid, String serial) {
        ...
        telldusServer.getStringFromService(msg);
    }
    ...
    public List<Sensor> tdSensor() {
        ...
        String retval = telldusServer.getStringFromService(msg);
     ...
     */
    public Protocol.ErrorCode tdSensorValue(String protocol, String model, int id, int dataType, SensorValue value) {
        assert value != null;
        ...
        String retval = telldusServer.getStringFromService(msg);
        if(retval == null || retval.length() == 0)
            return Protocol.ErrorCode.ERROR_METHOD_NOT_SUPPORTED;
     ...
     */
    public Protocol.ErrorCode tdSetControllerValue(int controllerId, String name, String value) {
     ...
     */
    public Protocol.ErrorCode tdRemoveController(int controllerId) {
